%% BIOE464 Monte Carlo Project, MC Simulation for Question 6
...
Nstep = 200000/N;            %simulation steps
sigma=0.08;                 %standard deviation for Gaussian distribution 
counter=0;                  %To count the accpected moves and energy
...
for trial = 1:Nstep
    ...
    proposed_coordinate_lattice = Gaussian_displace_particle(1,all_current_coords,L,sigma);
    
    for particle = 2:N
        % Simulate proposed movement of single particle
        proposed_coordinate_lattice = Gaussian_displace_particle(particle,proposed_coordinate_lattice,L,sigma);
    end
    ...
    for particle=1:N
        ...
        particle_coords=all_current_coords(:,particle);
        % Accept/reject based on Boltzmann
        [updated_coord updated_energy] = accept_reject(particle_coords, proposed_movement,particle_energy, proposed_energy, b);    %accept/reject based on Boltzmann factor
        
        %To count the accepted energy
        if updated_energy~=particle_energy
            counter=counter+1;
        end
...
toc
acceptance_rate=counter/(500*Nstep);
